...
if st.session_state['current_page'] == "resume":
    ...
    st.markdown("""
        ...
        '왜 수많은 직장 중에서 뉴로핏을 택했나'에 대한 분명한 이유를 가지고 있을수록 회사에 대한 애사심과 충성심이 높은 인재가 됩니다. 
        이 부분은 다른 회사에 넣었던 이력서를 제출한 것인지, 우리 회사의 정보와 맞추어 이력서를 작성했는지, 자기소개서의 지원 동기, 
        장래 희망 부분을 확인하여 동기가 확실한 인재인지 판단하시면 됩니다.

        ② 직무 적합성 평가

        이력서/자기소개서/포트폴리오를 통해 회사나 팀에서 요구하는 기술이나 기능에 대하여 지원자가 어느 정도 수준을 갖추고 있고, 
        이미 경력이 있다면 그 경험 중에서 어느 부분이 새로운 직무에 적용 가능한 것인지, 어떤 가치 창출이 가능한지 가늠해 봅니다.

        ③ 회사와 개인의 문화 적합도 평가

        조직의 문화와 개인의 특성 간 핏이 잘 맞아야 합니다. 조직의 문화적 특성이 맞는지 아닌지에 따라 같은 인재라도 성과가 달라질 수 있습니다. 
        조직의 핵심 가치인 도전정신, 협력 그리고 전문성, 책임감, 자신감, 적극성을 갖추고 있는지를 판단합니다.

        ④ 지원자에 관한 기초정보 자료 완성

        위의 사항 외에 개인의 비전, 잠재능력, 특이능력(외국어) 등 향후 회사에 도움이 되는 부분이 어느 정도인지 파악해 기초 정보를 종합적으로 완성합니다. 
    ...
    st.markdown("""
        ...
        인사팀에서 서류접수 확인과 기본사항 검토 후 현업 담당자님께 세부검토를 요청드립니다.
        - 뉴로핏 채용사이트 지원자 : 인사팀에서 이메일로 이력서 URL 전달
        - 특정 잡사이트 지원자 : 원티드 등 현업에서 사이트 접속해서 확인해야 하는 경우에는 별도 안내

        ② 서류검토 및 회신

        서류검토 및 전형 진행 결과는 인사팀 이메일(hrteam@neurophet.com)로 3일 이내 회신을 부탁드립니다.

        ③ 면접일정 확인

        합격자의 경우 인사팀에서 현업과 지원자의 일정을 확인해 1차 면접일정을 확인하고, 서류불합격자의 경우 인사팀에서 지원자에게 이메일로 통보합니다.
        """)
