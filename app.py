...
질문은 "상황(Situation), 과제(Task), 행동(Action), 결과(Result)"의 흐름이 드러날 수 있도록 하며, 각 항목에 맞춰 아래 기준으로 질문을 생성합니다.

[출력 형태]  
항목별 질문 리스트 (각 질문은 STAR 기반으로 구성)

---

🔹 1. 업무 지식 (8개 내외)
- ex) 뇌영상 처리 관련 기술을 적용했던 프로젝트 경험에서 직면했던 어려운 상황과, 이를 어떻게 해결했는지 말씀해주세요.

🔹 2. 직무기술 (업무경험 및 능력, 8개 내외)
- ex) Python 또는 MATLAB을 활용하여 데이터를 분석했던 경험 중, 가장 도전적인 과제와 그 해결 방식을 공유해주세요.

🔹 3. 직무 수행 태도 및 자세 (8개 내외)
- ex) 공동의 목표를 달성하기 위해 팀원과 갈등을 해결했던 경험이 있다면 말씀해주세요.
...
[주의사항]  
- 각 질문은 반드시 경험을 이끌어내는 STAR 구조로 유도할 것  
- 전문용어는 채용공고 기준으로 사용  
- 질문 수는 범위 내에서 조정 가능 (업무 지식/직무기술/태도: 각 8개 내외, 인성 질문: 5개 이내)"""},
                            {"role": "user", "content": f"이력서 내용:\n{text}\n\n채용공고:\n{job_description}\n\n위 내용을 바탕으로 STAR 기법에 기반한 면접 질문을 생성해주세요."}
                        ]
                    )
